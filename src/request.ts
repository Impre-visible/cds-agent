...
export async function buildRequest(todo: Todo, bot: GitLabUser): Promise<BuildResult> {
  if (!RELEVANT_ACTIONS.has(todo.action_name)) {
    return { ok: false, reason: `action « ${todo.action_name} » hors périmètre` };
  }

  const kind = resourceKind(todo.target_type);
  if (!kind) return { ok: false, reason: `cible « ${todo.target_type} » non gérée` };

  const projectId = todo.project?.id;
  const iid = todo.target?.iid;
  if (!projectId || !iid) return { ok: false, reason: "projet ou iid absent du to-do" };
  ...
  if (noteId === null) {
    ...
    if (note.author.id === bot.id) return { ok: false, reason: "note écrite par le bot lui-même" };
    text = note.body;
  ...
  if (!text.includes(`@${config.botUsername}`)) {
    return { ok: false, reason: "le texte ne mentionne pas le bot (réponse dans un thread ?)" };
  }

  return {
    ok: true,
    request: {
      key: noteId === null ? `desc:${projectId}:${kind}:${iid}` : `note:${noteId}`,
      todoId: todo.id,
      projectId,
